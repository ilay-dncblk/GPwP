...
kelimeler = ["kıbrıs", "everest", "afrika", "haliç", "mars", "istanbul", "ankara", "izmir", "şanlıurfa","ısparta"]
anlamlar =["Yavru vatan", "Dünyanın tepesi" , "Kara kıta", "Altın boynuz", "Kızıl gezegen" ,"Yedi tepeli şehir" ,"Türkiye’nin kalbi" , "Ege’nin İncisi" ,"Peygamberler şehri" ,"Güller diyarı" ]
resim = ["""
   +---+
   |   |
       |
...
def oyun_hazirlik():
    ...
    global secilen_kelime, gorunen_kelime, can , adim
...
def harf_al():
    ...
    while devam:
        ...
        if harf.lower() == "quit":
            cprint("Sizinle oyun oynamak bir zevkti.", color="red", on_color="on_blue")
            exit()
...
def oyun_dongusu():
    ...
    global gorunen_kelime, can , adim
    adim=0
    cprint("anlam: " + anlamlar[kelimeler.index(secilen_kelime)], color="red", attrs=["bold"])
    while can > 0 and secilen_kelime != "".join(gorunen_kelime):
        print(resim[adim])
        cprint("kelime: " + "".join(gorunen_kelime), color="red", attrs=["bold"])
        cprint("can   : <" + "❤" * can + " " * (5 - can) + ">", color="red", attrs=["bold"])
